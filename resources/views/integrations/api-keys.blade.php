...
@section('content')
    ...
    <div class="flex items-center justify-between mt-3">
        ...
        <!-- API Token para extensión de Chrome / Google Meet -->
        <div class="mt-10 bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 rounded-xl p-6 hover:border-orange-500/30 transition-all duration-300">
            <div class="flex items-center gap-3 mb-4">
                <div class="bg-gradient-to-br from-orange-500/20 to-red-600/20 p-2 rounded-lg">
                    <img src="/logos/google-meet.svg" class="h-5 w-5" alt="Google Meet" onerror="this.style.display='none'">
                </div>
                <div>
                    <h3 class="text-white font-medium">Token API (Extensión de Chrome / Google Meet)</h3>
                    <p class="text-gray-400 text-sm">Usa este token para conectar la extensión de Chrome que captura tus reuniones de Google Meet automáticamente.</p>
                </div>
            </div>

            @if(auth()->user()->api_token)
                <div class="flex items-center gap-2 bg-gray-900/60 border border-gray-700 rounded-lg p-3">
                    <input id="apiTokenInput"
                           type="password"
                           value="{{ auth()->user()->api_token }}"
                           readonly
                           class="flex-1 bg-transparent text-gray-300 text-sm font-mono outline-none">
                    <button onclick="toggleTokenVisibility()" title="Mostrar/ocultar"
                ...
                <div class="mt-3 flex items-center justify-between">
                    <p class="text-xs text-gray-500">Endpoint: <code class="text-orange-400">POST {{ url('/api/upload') }}</code></p>
                    <form method="POST" action="{{ route('api-token.regenerate') }}" onsubmit="return confirm('¿Regenerar el token? La extensión dejará de funcionar hasta que lo actualices.')">
                        @csrf
            ...
            @else
                <div class="text-center py-4">
                    <p class="text-gray-400 text-sm mb-3">No tienes token API generado todavía.</p>
                    <form method="POST" action="{{ route('api-token.regenerate') }}">
                        @csrf
                        <button type="submit"
                                class="px-4 py-2 bg-gradient-to-r from-orange-500 to-red-500 text-white text-sm font-medium rounded-lg hover:opacity-90 transition-opacity">
                            Generar token API
                        </button>
                    </form>
                </div>
    ...
    <script>
        ...
        function copyToken() {
            ...
            navigator.clipboard.writeText(input.value).then(() => {
                Swal.fire({ icon: 'success', title: '¡Copiado!', text: 'Token copiado al portapapeles.', timer: 1500, showConfirmButton: false });
            });
        }
